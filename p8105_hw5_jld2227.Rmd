...
spaghetti_facet <- spaghetti_con_exp + 
  facet_grid(~arm) + #creates a faceted version
  labs(title = "Subject Study Measurements by Week, in Control and Treatment")
library(patchwork)
spaghetti_con_exp / spaghetti_facet #plots the output
...
Tulsa, Alabama is a nonexistent city as per the [US census.]:https://factfinder.census.gov/faces/tableservices/jsf/pages/productview.xhtml?src=bkmk and represents an error in the dataset. The geo coordinates suggest the homicide may have occurred in Tulsa, Oklaholma instead.